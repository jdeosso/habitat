...
#    df_precios['por'] = df_precios['por']*df_precios['por_total']/100
    ...
    for index, row in df_rentabilidad1.iterrows():
        if (i==1):
            ...
            df_r1 = pd.DataFrame([[df_rentabilidad1.index[i], renta_acu]]) 
...
def objeto_final(resultados12, resultados36, resultados60, monto, rut, nombre_cliente, fondos):
    return dict({
      ...
      "rentabilidad_real_anualizada": {
        "m12": locale.currency(resultados12['rentabilidad_cartera'] * 100, grouping=True, symbol=''),
        "m36": locale.currency(resultados36['rentabilidad_cartera'] * 100, grouping=True, symbol=''),
        "m60": locale.currency(resultados60['rentabilidad_cartera'] * 100, grouping=True, symbol='')
      },
      "volatilidad": {
        "m12": locale.currency(resultados12['volatilidad_cartera'] * 100, grouping=True, symbol=''),
        "m36": locale.currency(resultados36['volatilidad_cartera'] * 100, grouping=True, symbol=''),
        "m60": locale.currency(resultados60['volatilidad_cartera'] * 100, grouping=True, symbol='')
      },
      "rentabilidad_real": {
        "m12": locale.currency(resultados12['rentabilidad_total'] * 100, grouping=True, symbol=''),
        "m36": locale.currency(resultados36['rentabilidad_total'] * 100, grouping=True, symbol=''),
        "m60": locale.currency(resultados60['rentabilidad_total'] * 100, grouping=True, symbol='')
      },
      "monto_proyectado": {
...
if __name__== "__main__":
    ...
    res = objeto_final(resultados12, resultados36, resultados60, monto, rut, nombre_cliente, lista)
