...
df_comicion_obli=pd.DataFrame([['CAPITAL-A','Obl','1.44'],['CAPITAL-B','Obl','1.44'],['CAPITAL-C','Obl','1.44'],['CAPITAL-D','Obl','1.44'],['CAPITAL-E','Obl','1.44'],
                                ['CUPRUM-A','Obl','1.44'],['CUPRUM-B','Obl','1.44'],['CUPRUM-C','Obl','1.44'],['CUPRUM-D','Obl','1.44'],['CUPRUM-E','Obl','1.44'],
                                ['HABITAT-A','Obl','1.27'],['HABITAT-B','Obl','1.27'],['HABITAT-C','Obl','1.27'],['HABITAT-D','Obl','1.27'],['HABITAT-E','Obl','1.27'],
                                ['MODELO-A','Obl','0.77'],['MODELO-B','Obl','0.77'],['MODELO-C','Obl','0.77'],['MODELO-D','Obl','0.77'],['MODELO-E','Obl','0.77'],
                                ['PROVIDA-A','Obl','1.45'],['PROVIDA-C','Obl','1.45'],['PROVIDA-D','Obl','1.45'],['PROVIDA-B','Obl','1.45'],['PROVIDA-E','Obl','1.45'],
                                ['PLANVITAL-A','Obl','1.16'],['PLANVITAL-C','Obl','1.16'],['PLANVITAL-D','Obl','1.16'],['PLANVITAL-B','Obl','1.16'],['PLANVITAL-E','Obl','1.16']
                                ],columns=['RUN','Serie','COM'])

#lista = [ '2018']
lista = list(range(2024 ,2025))
...
def ListaMultifondo_vc(df_mutlifondo_V):
    ...
    while z < len(list_multifondo):
        if list_multifondo[z].find(' Valor Cuota') > 1:
            lista_multifondo2.append(list_multifondo[z][:list_multifondo[z].find(' Valor Cuota')])
        z = z+1
    ## agrega serie
    ...
    df_mutlifondo1_V = df_mutlifondo1_V.filter(['Administradora', 'Nombre', 'RUN', 'serie','Tipo'])
    ...
    for x in tipo:
        ...
        while y < len(lista_multifondo2):
            ...
            df_temp = df_temp.rename(columns = {'serie': 'Serie', lista_multifondo2[y]+' Valor Cuota':'valor_cuota'})
            df_temp['Tipo'] = x[0]
...
def ListaMultifondo_serie_AV(df_afp_comi_av1p):
    ...
    for x in tipo_fondo:
        ...
        frames = frames.append(df_afp_comi_av11, ignore_index = True, sort = False)
...
def ListaMultifondo_serie_APV(df_mutlifondop):
    ...
    for x in tipo_fondo:
        ...
        frames = frames.append(df_afp_comi_apv11, ignore_index = True, sort = False)
    return(frames)
#####inicio codigo
if __name__ == "__main__":
    ...
    df_fondosmutuos_c = pd.read_json(file_tabla_comision,lines=True)
...
#    obj = bucket.Object(key=file_habitat_sii_dolar)
#    js_mutlifondo1 = obj.get()['Body'].read().decode('utf-8')
#    df_dolar = pd.read_json(js_mutlifondo1, lines=True)
#    
#    obj = bucket.Object(key=file_habitat_sii_uf)
#    js_mutlifondo1 = obj.get()['Body'].read().decode('utf-8')
#    df_uf = pd.read_json(js_mutlifondo1, lines=True)
#    
#    
...
#    
#    obj = bucket.Object(key=file_habitat_comisiones_ahorro_voluntario)
#    js_mutlifondo1 = obj.get()['Body'].read().decode('utf-8')
#    df_afp_comi_av = pd.read_json(js_mutlifondo1, lines=True)
#
#    obj = bucket.Object(key=file_habitat_comisiones_apv)
#    js_mutlifondo1 = obj.get()['Body'].read().decode('utf-8')
#    df_afp_comi_apv = pd.read_json(js_mutlifondo1, lines=True)
#    
#    obj = bucket.Object(key=file_habitat_valores_cuota_multi_fondo)
#    js_mutlifondo1 = obj.get()['Body'].read().decode('utf-8')
#    df_mutlifondo_tot = pd.read_json(js_mutlifondo1, lines=True)
...
#    
    ...
    while i < len(lista):
        ...
        df_mutlifondo = df_mutlifondo_tot[(df_mutlifondo_tot['Fecha'].astype(str).str[:4] == str(lista[i]))]    
        ...
        dd_fondosmutuos = inbag.to_dataframe(columns = ['RUN','Serie','Valor cuota','date','Moneda en que contabiliza el fondo'])
        df_fondosmutuos = dd_fondosmutuos.compute()
        
        df_mutlifondo = df_mutlifondo_tot[(df_mutlifondo_tot['Fecha']<=  fin.isoformat())]    
    #    dd_fondosmutuos = records1.to_dataframe(meta=None,columns = ['RUN','Serie','Valor cuota','date','Moneda en que contabiliza el fondo'])
        ...
        df_multifondo3 = pd.merge(df_multifondo2,df_afp_comi_av1,on = ['mes','RUN','Serie'],how = 'left')
        ...
        df_multifondo4 = pd.merge(df_multifondo3,df_afp_comi_apv1,on = ['mes','RUN'],how = 'left')
        df_multifondo4['tac_total'] = np.where(df_multifondo4['value'].isnull(),np.where(df_multifondo4['value1'].isnull(),0,df_multifondo4['value1']),df_multifondo4['value'])
        
        df_multifondo4 = df_multifondo4.filter(['RUN','Serie_x','valor_cuota','Fecha','uf','tipo_cambio','tac_total'])
        dd_multifondo4 = dd.from_pandas(df_multifondo4, npartitions = 3)
        dd_multifondo4 = dd_multifondo4.rename(columns = {'Serie_x':'Serie'})
        ##### vc fm
        dd_fondosmutuos1 = dd_fondosmutuos.rename(columns = {'serie': 'Serie','Valor cuota':'valor_cuota','date':'Fecha','Moneda en que contabiliza el fondo':'Moneda'})
        ### salvo dd procesado
        ### convierto pd a dd
        ...
        dd_fondosmutuos3 = dd_fondosmutuos3[['RUN','Serie','valor_cuota','Fecha','uf','tipo_cambio','tac_total']]
        
        #### union dos df
        dd_result = dd_multifondo4.append(dd_fondosmutuos3)
        ...
        for index,row in df_comicion_obli.iterrows():
            ...
            df_result['tac_total']=np.where(((df_result['RUN']==run)& (df_result['Serie']==serie) ),com, df_result['tac_total'])
        ...
        for index, row in df_result_run_serie.iterrows():
            ...
            file_csv=df_result1.to_csv('{}{}-{}.csv'.format(file_precios, row['RUN'], lista[i]), index=False)
